...
export class NotificationsService {
    ...
    async findAll(userId: string): Promise<Notification[]> {
        return this.notificationModel.find({_id: new Types.ObjectId(userId)}).sort({ date: -1 }).exec()
    }
    ...
    async update(id: string, updateDto: UpdateNotificationDto, userId: string): Promise<Notification>{
        const notification = await this.findOne(id, userId) as NotificationDocument
